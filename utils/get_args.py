...
def get_args():
    ...
    parser.add_argument("--noise-ratio", type=float, default=0, help="the ratio of correspondence noise.")
    ...
    parser.add_argument("--lr", type=float, default=0.00025, help="learning rate for other modules in MITH.")

    parser.add_argument("--is-train", action="store_true")

    parser.add_argument("--hyper-alpha", type=float, default=10, help="$\ alpha$ in paper.")
    parser.add_argument("--hyper-beta", type=float, default=8, help="weight of the quantization loss, which is labeled as $\ gamma$ in paper.")
    parser.add_argument("--hyper-lambda", type=float, default=0.99, help="$\ lambda$ in paper.")
    parser.add_argument("--hyper-gamma", type=float, default=500, help="")
    parser.add_argument("--hyper-zeta", type=float, default=50,
                        help="weight of the contrastive inter-modal alignment loss, which is labeled as $\ delta$ in paper.")

    # other
    parser.add_argument("--clip-path", type=str, default="./cache/ViT-B-32.pt", help="pretrained clip path.")
    parser.add_argument("--frozen-clip-path", default="optional/path", help="pretrained clip model of MITH.")
    parser.add_argument("--frozen-hash-path", default="optional/path", help="pretrained hash model of MITH.")
    ...
    parser.add_argument("--img-caption-file", type=str, default="image_caption.mat")
